...
(function() {
    ...
    TypewriterPrototype.deleteUntilMarker = function() {
		this._addToEventQue([this._deleteChars, ['until']]);
		return this;
    }
	...
	TypewriterPrototype._deletingCharAnimationFrame = function() {
        ...
        if(!self._settings.charAmountToDelete 
            || self._settings.charAmountToDelete === 0
            || current_typed_char_ids === 0) {
        ...
        if (char_amount_to_delete === 'until') {
            const mark = self.el.querySelector('.mark');
            const wrapper = mark.parentNode;
            debugger;
        }
